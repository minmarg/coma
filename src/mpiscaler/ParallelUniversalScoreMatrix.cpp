...
// -------------------------------------------------------------------------
// TellSlavesToTerminate: broadcasts message to the slave processes to tell
...
// -------------------------------------------------------------------------
// ScaleScoringMatrix: the master process scales scoring matrix using the
//     slave processes
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// ComputeScore: compute score given query and subject charactersitic
//     vectors of frequencies
// -------------------------------------------------------------------------

double ParallelUniversalScoreMatrix::ComputeScore(
    const FrequencyVector& vect_1st,
    const FrequencyVector& vect_2nd ) const
{
    double  score = 0.0;
...
// -------------------------------------------------------------------------
// VectorScoreProbability: compute score probability given query and subject
//     charactersitic vectors of frequencies; computation depends on vector
//     distribution type chosen
// -------------------------------------------------------------------------

double ParallelUniversalScoreMatrix::VectorScoreProbability(
    const FrequencyVector& rowvect,
    const FrequencyVector& colvect ) const
{
...
// -------------------------------------------------------------------------
// MultiplierProgress: method which is invoked just before saving of the
//     multiplier; if master it prints information of the parameters
// -------------------------------------------------------------------------
...
//         for( size_t M = N; M < no_freqs && !error; M ++ )
        for( size_t M = N + GetMyMPIRank() - 1; M < no_freqs && !error; M += GetMPIRingSize() - 1 )
        {
            ...
            score = ComputeScore( rowvector, colvector );
            scoreprob = VectorScoreProbability( rowvector, colvector );
            ...
            if( M != N ) {
                ...
                scoreprob = VectorScoreProbability( colvector, rowvector );
...
void ParallelUniversalScoreMatrix::PrintScoringMatrix( FILE* fp ) const
{
    if( !GetStore() || !GetStore()->GetFrequencies())
        return;
    ...
    for( size_t N = 0; N < no_freqs; N++ ) {
        ...
        {
            const FrequencyVector   colvector(( const char* )frequencies->GetValueAt( M ));
            fprintf( fp, "%3d ", ( int )rint( ComputeScore( rowvector, colvector )));
        }
    }
