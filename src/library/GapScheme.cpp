...
void GapScheme::Push( double w, double delbeg, double delend, int delta, char aa )
{
    if( allocated < GetColumns() + 1 ) {
        reallocate(  allocated * 2 );
    }

    PushAt( w, delbeg, delend, delta, aa, GetColumns());
}
...
void GapScheme::PushAt( double w, double delbeg, double delend, int delta, char aa, int position )
{
    ...
    double  relent )
{
    ...
    SetThickness( logo.GetMtxEffectiveThickness());
...
void GapScheme::OutputGapScheme( const char* filename )
{
    ...
    if( fp == NULL )
        throw myruntime_error(
            mystring( "Failed to open file for writing gap costs." ));

    fprintf( fp, "Gap opening cost, %d\nExtending gap cost, %d\n\n", ( int )rint( GetOpenCost()), ( int )rint( GetExtendCost()));
    fprintf( fp, "%5c Position-specific gap \n %8c costs    weights     deletions / int  slope", 32, 32 );

    for( int p = 0; p < GetColumns(); p++ ) {

        fprintf( fp, "\n%5d %c   ", ++l, DehashCode( AAcid( p )));
        fprintf( fp, "%4d  %9.3f %6.3f - %-6.3f / %-3d %6.3f",
...
void GapScheme::CheckIntegrity() const
{
    for( int n = 0; n < GetColumns(); n++ )
        if( NUMALPH <= AAcid( n ) ||
            1.0 < GetWeightsAt( n ) || GetWeightsAt( n ) < 0.0,
            1.0 < GetDeletesBegAt( n ) || GetDeletesBegAt( n ) < 0.0 ||
            1.0 < GetDeletesEndAt( n ) || GetDeletesEndAt( n ) < 0.0 ||
            GetDeletesIntervalAt( n ) < 0
        )
            throw myruntime_error(
                mystring( "GapScheme: Data corrupted." ));
}
