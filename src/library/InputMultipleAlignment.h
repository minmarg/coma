...
protected:
    ...
    double                      ExpectedNoObservationsAt( size_t pos ) const;
...
//     bool            NotNull( size_t n ) const { return SequenceAt( n ) != NULL; }
    ...
    size_t                      GetEffectiveSize() const        { return effective_length; }
    void                        SetEffectiveSize( size_t len )  { effective_length = len;  }
...
private:
    ...
    size_t                  effective_length;       //effective number of sequences used to compute profile scores
    //
