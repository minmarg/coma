...
public:
    ...
    virtual void        PrintScoringMatrix( FILE* ) const;      //print the computed scoring system
...
bool UniversalScoreMatrix::PairThicknessConstraintsMet( int m, int n ) const
{
    ...
    if( !GetSbjctLogo().GetMtxEffectiveThickness() || !GetQueryLogo().GetMtxEffectiveThickness())
        throw myruntime_error( mystring( "UniversalScoreMatrix: Profile thickness found to be wrong." ));
#endif

    if(           GetSbjctLogo().GetThicknessAt( m ) < ( size_t )GetThicknessNumber() ||
                  GetQueryLogo().GetThicknessAt( n ) < ( size_t )GetThicknessNumber() ||
        ( double )GetSbjctLogo().GetThicknessAt( m ) / GetSbjctLogo().GetMtxEffectiveThickness() < GetThicknessPercents() ||
        ( double )GetQueryLogo().GetThicknessAt( n ) / GetQueryLogo().GetMtxEffectiveThickness() < GetThicknessPercents())
