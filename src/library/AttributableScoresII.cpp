...
void AttributableScoresII::ScaleToAttainLambda()
{
    ...
    bool    USE_BOUND_PRECOMPUTATION = true;

    double  prev_lambda;    //previously computed lambda
    double  diff_lambda;    //difference of the consecutive lamdas
    double  diff_target;    //difference of the target lambdas
    double  min_diff_target = 10.0;//minimum value of such difference
    ...
    bool    greater = GetLambda() > GetRefLambda();
...
// fprintf( stderr, "factor=%f, factor_low=%f, factor_high=%f\n", multiplier, left, right );
    ...
    if( GetLambda() > 0.0 )
        for( int i = 0; i < MAX_SCALE_ITERATIONS; i++ ) {
...
// fprintf( stderr, "factor=%f, factor_low=%f, factor_high=%f\n", multiplier, left, right );
            ...
            //even small difference may imply better accuracy...
    ...
    if( GetLambda() < 0.0 ) {
        ...
        //when expected score is near zero. In this case, approximate integer scaling
        //is used
// fprintf( stderr, "IntegerScaleMatrix...\n" );
        AdjustScoresInMatrix( multiplier = 1.0, ImageTable );
...
double AttributableScoresII::IntegerScaleMatrix()
{
    ...
    while( GetLambda() > 0.0 && iter++ < MAX_SCALE_ITERATIONS )
    {
        prev_lambda = GetLambda();
