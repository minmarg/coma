    ...
    double          info )
{
    ...
    if( MAXTHCKVALUE < thickness )
        thickness = MAXTHCKVALUE;

    SetWeight(( WGHTTYPE )rint( weight ));
    SetThickness(( THCKTYPE ) thickness );
...
//     int             weight, thickn, info;
    ...
    if(( emsg = read_double( p, length - size_t( p - locbuffer ), &prob, &rbts )) != NULL )
        throw myruntime_error( emsg );
...
// -------------------------------------------------------------------------
// FrequencyValueComparer: Compare values for two frequency vectors
// -------------------------------------------------------------------------
