...
double ScoringMatrix::ComputeScore( int m, int n, bool final ) const
{
    ...
    if( final ) {
        fst_thckn = GetQueryLogo().GetThicknessAt( n );
        sec_thckn = GetSbjctLogo().GetThicknessAt( m );


//         if( GetQueryLogo().GetMtxEffectiveThickness() &&
//             GetSbjctLogo().GetMtxEffectiveThickness())
//         {
//             log_fst_thckn = ( double )fst_thckn / ( double )GetQueryLogo().GetMtxEffectiveThickness();
//             log_sec_thckn = ( double )sec_thckn / ( double )GetSbjctLogo().GetMtxEffectiveThickness();
//         }
//         else {
...
// -------------------------------------------------------------------------
// PreliminaryVerification: applies fast alignment algorithm given this
//     score system before scaling takes place
// -------------------------------------------------------------------------
...
//                     sum += ( freq_fst_( n, r ) + freq_sec_( m, r )) * LOSCORES.LogPROBABility( r );
            ...
            if( normterm ) {
                ...
                if( consval < 0.999 || consval > 1.001 )
                    warning( "ScoringMatrix: ComputePositionalScoreProbs: Probability conservation broken." );
            }
        }
...
void ScoringMatrix::PrintScoringMatrix( FILE* fp ) const
{
    int l = 0;
    int m, n;
