...
void AttributableScores::ComputeStatisticalParameters( bool computelambda )
{
    if( computelambda )
...
//
void AttributableScores::ComputeProbabilitiesAndLambda()
{
    ComputeScoreProbabilities();
    if( GetExpectedScore() < 0.0 )
        ComputeScalingLambda();
...
void AttributableScores::ComputeScalingLambda()
{
    ...
    if( 0.0 <= GetExpectedScore()) {
        ...
        if( newlambda <= 0.0 + EPSILON ) {
            ...
            warning( "Computation of statistical parameter lambda failed." );
            //return;
        }
    }
