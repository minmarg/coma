...
void ProfileMatrix::PrintScoringMatrix( FILE* fp ) const
{
    int l = 0;
...
// ==== Functions ==========================================================
// ComputedBlosum62WithParameters: outputs computed Blosum62 table with
//     computed statistical parameters for it
//
void ComputedBlosum62WithParameters()
{
    ...
    ProfileMatrix   blosum_mat( COMPUTED_BLOSUM62.data, resids, NUMAA, config, AbstractScoreMatrix::NoScaling );
    blosum_mat.ComputeStatisticalParameters();
//     blosum_mat.ScaleScoringMatrix(); //if scaling needed
    blosum_mat.PrintScoringMatrix( stderr );
    dynamic_cast<AbstractScoreMatrix&>( blosum_mat ).PrintParameterTable( stderr );
}
