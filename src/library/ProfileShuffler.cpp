...
void ProfileShuffler::SimulateProfile( FrequencyMatrix& freq, LogOddsMatrix& pssm, GapScheme& gaps )
{
    ...
    pssm.SetMtxThickness( thick );
    pssm.SetMtxEffectiveThickness( thick );
    ...
    for( int n = 0; n < GetProfileLength(); n++ )
    {
        ...
        pssm.PushAt( pospssmvalues, loc_residues[n], vector.GetWeight(), vector.GetInfContent(), vector.GetThickness(), n );
        gaps.PushAt( gapweight, delbegwht, delendwht, deldelta, loc_residues[n], n );
    }
}
