...
#include "HashTable.h"
#include "RJHashing.h"
#include "SBoxHashing.h"
#include "HashFunctions.h"
#include "MD5Hashing.h"
#include "CRCHashing.h"
...
#define THCKTYPE    unsigned short
#define THCKSIZE  ( sizeof( THCKTYPE ))
#define MAXTHCKVALUE 65535
...
class FrequencyVector
{
    ...
    enum {  preamble = sizeof( unsigned int ),          //dummy field for indication of frequency vector existance in memory
            ...
            thickSz  = THCKSIZE,                        //effective thickness frequency vector was constructed from
...
public:                                                 //for writing
    ...
    THCKTYPE    GetThickness() const;                   //get effective thickness
...
protected:
    ...
    void        SetThickness( THCKTYPE value );
...
private:
    ...
    SimpleVector*   frequencies;            //contains pointers to frequency vectors to be able to iterate over all of them
...
inline
THCKTYPE FrequencyVector::GetThickness() const
{
#ifdef __DEBUG__
    if( !vector )
        throw myruntime_error(
            mystring( "FrequencyVector: Memory access error." ));
#endif
    return *( THCKTYPE* )( vector + preamble + vectorSz + alphaSz );
}
...
inline
void FrequencyVector::SetThickness( THCKTYPE value )
{
#ifdef __DEBUG__
    if( !vector )
        throw myruntime_error(
            mystring( "FrequencyVector: Memory access error." ));
#endif
    *( THCKTYPE* )( vector + preamble + vectorSz + alphaSz ) = value;
}
...
//     values of the keys to be equal as well
// -------------------------------------------------------------------------
