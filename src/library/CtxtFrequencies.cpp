...
bool CtxtFrequencies::Read( FILE* fp )
{
    ...
    if(( emsg = skip_comments( fp, locbuffer, locsize, &len )) != NULL )
    ...
    for( r = 0; r < rescount; r++ ) {
        ...
        if(( emsg = read_symbol( locbuffer + read, len - read, &res, &rbts )) != NULL )
            throw myruntime_error( emsg );
    ...
    for( size_t n = 0; n < GetAllocated() && !feof( fp ); n++ )
    {
        ...
        for( r = 0; r < rescount; r++ )
        {
            ...
            //read frequency
            if(( emsg = read_double( locbuffer + read, len - read, &val, &rbts )) != NULL )
                throw myruntime_error( emsg );
            read += rbts;
...
// -------------------------------------------------------------------------
// Write: write double values to file
//
...
void CtxtProScores::Read( const char* filename )
{
    ...
    try {
        ...
        if(( emsg = skip_comments( fp, locbuffer, locsize, &len )) != NULL ) throw myruntime_error( emsg );
        if(( emsg = skip_comments( fp, locbuffer, locsize, &len )) != NULL ) throw myruntime_error( emsg );
        if(( emsg = skip_comments( fp, locbuffer, locsize, &len )) != NULL ) throw myruntime_error( emsg );
