...
// Constructor
//
Database::Database( const char* output, const char* directory )
:
    dbname( output ),
    data_dir( directory ),
    profiles( NULL ),
...
// Constructor
//
Database::Database( const char* output, char* arguments[], int no_args )
:
    dbname( output ),
    data_dir( NULL ),
    profiles( arguments ),
...
//         serializer.DeserializeProfile( freq, pssm, gaps, filename );//obsolete
        ...
        for( int n = 0; n < freq.GetColumns(); n++ )
        {
            ...
            FrequencyVector vect(   *freq.GetVectorAt( n ), pssm.GetFrequencyWeightAt( n ), pssm.GetThicknessAt( n ),
...
//         Serializer::Read( db_fp[FREQ], ( char* )&size, sizeof( size ), 1 );//obsolete
        ...
        if(( emsg = skip_comments( db_fp[FREQ], locbuffer, locsize, &length )) != NULL )
        ...
        if(( emsg = read_llinteger( p, length - size_t( p - locbuffer ), &llintval, &rbts )) != NULL )
            throw myruntime_error( emsg );
...
const char* patstrDBVER = "COMA/CONDOR profile database v";
