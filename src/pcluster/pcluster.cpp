...
int main( int argc, char *argv[] )
{
    ...
    while( 1 ) {
        ...
        if(( c = getopt_long_only( argc, argv, "hvi:o:c:p:Uw:f:F:", long_options, &option_index )) == -1 )
            break;
#else
        if(( c = getopt( argc, argv, "hvi:o:c:p:Uw:f:F:" )) == -1 )
            break;
#endif
    ...
    try {
        ...
        clusters->SetComputeDELETEstates( true );
